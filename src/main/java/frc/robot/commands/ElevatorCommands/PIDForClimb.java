...
public class PIDForClimb extends SequentialCommandGroup {
  /** Creates a new ManualUpDown. */
  public PIDForClimb(Elevator elevator, AlgaePivot algaePivot, double desiredHeight) {
    ...
    if (algaePivot.getPivotEncoder() > Constants.AlgaePivot.ENC_REVS_ELEVATOR_SAFE_POSITION){
      ...
      new DangerPIDForClimb(elevator, desiredHeight));
    }
    else{
      addCommands(
      new DangerPIDForClimb(elevator, desiredHeight));
